...
class BleManager(private val context: Context) {
    ...
    fun startScan(nameFilter: String?, timeoutSeconds: Int, result: MethodChannel.Result) {
        startScanWithRetry(nameFilter, timeoutSeconds, result, retryCount = 0)
    }

    private fun startScanWithRetry(nameFilter: String?, timeoutSeconds: Int, result: MethodChannel.Result, retryCount: Int) {
        ...
        if (bluetoothLeScanner == null) {
            // If scanner is null and Bluetooth is enabled, it might be initializing
            // Retry up to 3 times with 300ms delay
            if (retryCount < 3) {
                Log.w(tag, "Bluetooth LE scanner is null (attempt ${retryCount + 1}/3) - waiting for Bluetooth to initialize...")
                handler.postDelayed({
                    startScanWithRetry(nameFilter, timeoutSeconds, result, retryCount + 1)
                }, 150)
                return
            } else {
                Log.e(tag, "Bluetooth LE scanner is still null after 3 retries - Bluetooth initialization failed")
                result.error("BLUETOOTH_UNAVAILABLE", "Bluetooth LE scanner not available after retry", null)
                return
            }
        }
    ...
    fun discoverServices(deviceId: String, result: MethodChannel.Result) {
        ...
        }, 2500) // 2.5 second timeout - faster response
    }
    ...
    fun setNotify(deviceId: String, serviceUuid: String, charUuid: String, enable: Boolean, result: MethodChannel.Result) {
        ...
        try {
            ...
            // Minimal delay to avoid BLE command queue conflict
            Thread.sleep(30)
